...
UPDATE inventory SET inv_year = '2024' WHERE inv_id = 1;

DELETE FROM inventory WHERE inv_description = '"If you want to look cool this is the ar you need! This car has great performance at an affordable price. Own it today!"';

-- SQL statements to accomplish the following tasks. 
INSERT INTO account 
...
UPDATE 	inventory SET inv_description = '"Do you have 6 kids and like to go offroading? The Hummer gives you a huge interiors with an engine to get you out of any muddy or rocky situation."' WHERE inv_id = 10;

SELECT inv_make, inv_model, classification.classification_name FROM inventory INNER JOIN classification ON inventory.classification_id = classification.classification_id WHERE classification.classification_id = 2;

UPDATE inventory SET inv_image = CONCAT('/images/vehicles/', REPLACE(inv_image, '/images/', '')),
    inv_thumbnail = CONCAT('/images/vehicles/', REPLACE(inv_thumbnail, '/images/', ''));

--
--
-- SQL statement to rebuild database 
--
--
